    ...
    if request.method == "POST":
        ...
        if form.is_valid() and imagesForm.is_valid():
            ...
            for form in imagesForm.cleaned_data:
                if form:
                    picture = form["url"]
                    text = form['image_name']
                    newPicture = Photo(listing = newListing, url = picture, image_name = text)
                    newPicture.save()

            return render(request,"auctions/newListing.html",{
                "form": newListingForm(),
                "imageForm": newPictureForm(),
                "success": True
            })
        else:
            return render(request,"auctions/newListing.html",{
...
def watchlist(request):
    ...
    for listing in listings:
        listing.mainPicture = listing.photos.first()
        if request.user in listing.watchers.all():
            listing.is_watched = True
        else:
            listing.is_watched = False
